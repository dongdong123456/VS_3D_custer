...
def encode_framing(payload):
    ...
    s1= chr(chksum_cola(payload))
    s2=len(payload)

    return START_STX + struct.pack('>I', len(payload)) + payload + bytes(chr(chksum_cola(payload)),encoding='utf-8')
...
def to_hex(str_value):
    ...
    return ' '.join(str(x) for x in str_value)#.encode('hex')
...
class Control:
    ...
    def sendToDevice(self, message, extra_bytes):
        ...
        time.sleep(1.001) # at least the header should be receivable
    ...
    def sendCoLaB(self, cmd, mode, payload):
        msg = self._COLAB_HEADER.pack(b's', cmd, mode) +b' '+ payload
    ...
    def sendCommand(self, cmd, name, payload=None):
        ...
        payload = bytes(name) + bytes(b' ') + bytes(payload)
